...
namespace GaudiMath{
  ...
  */
  ...
  Vec4 rayPlaneIntersect(Vec4 l0, 
			 Vec4  r, 
			 Vec4  N, 
			 Vec4  p){
  ...
  Vec4  projectToLine(Vec4  p, 
    ...
    Vec4 projdlpdl = dl.dot(dlp)*normalize(dl); //project vector onto the normal
  ...
  Mat4 RandomRotation(){
    ...
    Quat Q = 
      Quat (Eigen::AngleAxisf(fRand(0, 2.0)*M_PI, 
						  Vec3(fRand(-1,1),
								  fRand(-1,1),
								  fRand(-1,1))));
    Q.normalize();
    Mat3 R3 = Q.matrix();
    Mat4 R4; R4.setIdentity(); R4.block<3,3>(0,0) = R3;
    return R4;
  }
