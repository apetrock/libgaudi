...
namespace GaudiGraphics {
  ...
  public:
    ...
    using OnFrameCallback = std::function<void(const int & frame,
			     BufferObject & buffer,
			     Viewer & context,
			     void * data)>;

    std::vector<OnFrameCallback> onFrameCallbacks;
    
    void onFrame(const int & frame,
           BufferObject & buffer,
           Viewer & context,
           void * data){
      //call onframe callbacks
      for (auto & callback : onFrameCallbacks){
      callback(frame, buffer, context, data);
      }
  ...
  public:
    ...
    static ImmediateLinesPtr create(){return std::make_shared<ImmediateLines>();}
