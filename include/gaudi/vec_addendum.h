...
namespace va
{
  ...
  template <typename T>
  T sgn(const VEC3<T> &v0, const VEC3<T> &v1)
  {
  ...
  template <class Tf, class Tv>
  inline Tv mix(Tf f, const Tv &x0, const Tv &x1)
  {
    return (1.0 - f) * x0 + f * x1;
  }

  template <class Tf>
  inline bool mix(Tf f, const bool &x0, const bool &x1)
  {
    if (x0 && x1)
      return true;
    else if (!x0 && !x1)
  ...
  template <class T>
  inline T clamp(const T &v, const T &lb, const T &ub)
  {
  ...
  */
  ...
  template <int N, typename T>
  inline VEC<N, T> div(const VEC<N, T> &A, const VEC<N, T> &B)
  {
    return A.array() / B.array();
  }

  template <int N, typename T>
  inline VEC<N, T> mult(const VEC<N, T> &A, const VEC<N, T> &B)
  {
    return A.array() * B.array();
  }
  ...
  template <typename T>
  T norm(VEC3<T> a) { return a.norm(); };

  template <typename T>
  double norm(std::complex<T> a)
  {
    a = a * a;
  ...
  template <typename T>
  VEC3<T> normalize(VEC3<T> a) { return a / norm(a); };
  template <typename T>
  VEC4<T> normalize(VEC4<T> a) { return a / norm(a); };
...
T solidAngle(const VEC3<T> &pi, const VEC3<T> &p0, const VEC3<T> &p1,
  ...
  template <typename T>
  T dist_from_segment(const Eigen::Matrix<T, 2, 1> &x0,
                      const Eigen::Matrix<T, 2, 1> &x1,
                      const Eigen::Matrix<T, 2, 1> &pt)
  {
    T l2 = (x1 - x0).squaredNorm();
    if (l2 == 0)
      return (pt - x0).squaredNorm();
    T t = ((pt[0] - x0[0]) * (x1[0] - x0[0]) + (pt[1] - x0[1]) * (x1[1] - x0[1])) / l2;
    t = std::max(0.0, std::min(1.0, t));
    Eigen::Matrix<T, 2, 1> projection = x0 + t * (x1 - x0);
    return (pt - projection).norm();
  }

  template <typename T>
  T is_left(const Eigen::Matrix<T, 2, 1> &p0, const Eigen::Matrix<T, 2, 1> &p1, const Eigen::Matrix<T, 2, 1> &dx)
  {
    return (p1[0] - p0[0]) * (dx[1] - p0[1]) - (dx[0] - p0[0]) * (p1[1] - p0[1]);
  }

  template <typename T>
  T winding_number(const Eigen::Matrix<T, 2, 1> &dx, const std::vector<Eigen::Matrix<T, 2, 1>> & shape)
  {
    T wn = 0.0;
    int N = shape.size();
    for (int i = 0; i < shape.size(); i++)
    {
      Eigen::Matrix<T, 2, 1> p0 = shape[i];
      Eigen::Matrix<T, 2, 1> p1 = shape[(i + 1) % N];

      if (p0[1] <= dx[1])
      {
        if (p1[1] > dx[1])
        {
          if (is_left(p0, p1, dx) > 0.0)
          {
            wn += 1.0;
          }
        }
  ...
  template <typename T>
  MAT3<T> rejection_matrix(const VEC3<T> &N)
  {
    return MAT3<T>::Identity() - N * N.transpose();
  }

  template <typename T>
  MAT3<T> projection_matrix(const VEC3<T> &N)
  {
    return N * N.transpose();
  }

  template <typename T>
  MAT3<T> skew_symmetric_matrix(const VEC3<T> &x)
  {
  ...
  template <typename T>
  inline VEC3<T> project(const VEC3<T> &N, const T &A)
  {
    return A;
  };
  ...
  template <typename T>
  inline VEC3<T> calculate_normal(vector<VEC3<T>> vt_list)
  {
  ...
  template <typename T>
  inline VEC3<T> fnormalize(VEC3<T> vecin)
  {
  ...
  template <typename T>
  QUAT<T> slerp0(QUAT<T> a, QUAT<T> b, float t)
  {
    T dotAB = a.dot(b);
    dotAB = abs(dotAB);
    T s0, s1;
  ...
  template <typename T>
  QUAT<T> slerp(QUAT<T> a, QUAT<T> b, float t)
  {
    T dotAB = a.dot(b);
    // dotAB = abs(dotAB);
  ...
  template <typename T>
  MAT3<T> exp_slerp(MAT3<T> Ma, MAT3<T> Mb, T t)
  {
  ...
  */
  ...
  template <typename T>
  QUAT<T> exp_slerp(QUAT<T> qa, QUAT<T> qb, T t)
  {
    qa.normalize();
    qb.normalize();
    QUAT<T> qla = qlog(qa);
  ...
  template <typename T>
  VEC3<T> rgb_to_hsv(const VEC3<T> &rgb)
  {
  ...
  template <typename T>
  VEC3<T> hsv_to_rgb(const VEC3<T> &hsv)
  {
    VEC3<T> rgb;
    // From medit
    T f, p, q, t, hh;
  ...
  template <typename T>
  VEC3<T> rgb_to_xyz(const VEC3<T> &rgb)
  {
    // rgb is in 0-1 range
    VEC3<T> xyz;
  ...
  template <typename T>
  VEC3<T> xyz_to_rgb(const VEC3<T> &lab)
  {
    // rgb is in 0-1 range
    VEC3<T> rgb;
  ...
  template <typename T>
  VEC3<T> rgb_to_lab(const VEC3<T> &rgb)
  {
  ...
  template <typename T>
  VEC3<T> lab_to_rgb(const VEC3<T> &lab)
  {
    VEC3<T> xyz;
