...
enum class laplace_modes_band {
  ...
  /// uses `mid_slider_t` (see ctor). Explicit \f$\sigma \ge 0\f$ ignores the slider.
  shift_invert_middle,
};
...
public:
  ...
  spectral_modes_demo(int num_modes = 32, double epsilon_shift = 1e-6,
    ...
    if (_band == laplace_modes_band::low_frequency)
      ...
      if (sigma < 0.0) {
        ...
        const double sigma_hi = 0.35 * std::max(bound, 1e-12);
        const double sigma_lo = std::max(1e-12, 1e-8 * sigma_hi);
        if (_mid_slider_t >= 0.0 && _mid_slider_t <= 1.0) {
          const double t = std::clamp(_mid_slider_t, 0.0, 1.0);
          sigma = (1.0 - t) * sigma_lo + t * sigma_hi;
          std::cerr << "[spectral_modes_demo] mid-band auto shift sigma=" << sigma
                    << " (lerp t=" << t << " between sigma_lo=" << sigma_lo
                    << " and sigma_hi=" << sigma_hi << ")\n";
        } else {
          sigma = sigma_hi;
          std::cerr << "[spectral_modes_demo] mid-band auto shift sigma=" << sigma
                    << " (upper target 0.35 * max row-sum abs bound " << bound
                    << ")\n";
        }
      } else {
  ...
  /// Per-vertex colors: growth_study-style linear mix, driven by eigenmode scalar.
  /// Min–max by default. Tiny span → z-score contrast. When span ≫ σ (few vertices
  /// set min/max but bulk sits near 0), z-score would map almost everything to ~0.5;
  /// use a robust 2–98% range instead so localized / high-frequency modes stay visible.
  std::vector<vec4> get_mesh_colors() const {
    ...
    double mn = _evecs(0, col), mx = _evecs(0, col);
    for (int i = 1; i < n; ++i) {
      const double v = _evecs(i, col);
      mn = std::min(mn, v);
      mx = std::max(mx, v);
    }
    ...
    double var = 0.0;
    for (int i = 0; i < n; ++i) {
      const double d = _evecs(i, col) - mean;
      var += d * d;
    }
    ...
    for (int k = 0; k < n; ++k) {
      const double phi = _evecs(k, col);
      double t = 0.5;
      if (kind == scale_kind::z_score && sigma > 1e-14)
        t = (phi - mean) / (3.0 * sigma) + 0.5;
