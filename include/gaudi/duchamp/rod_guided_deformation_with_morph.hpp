  ...
  namespace duchamp
  {
    ...
    public:
      ...
      rod_guided_deformation_with_morph()
      {
        ...
        //__M = shell::load_obj("assets/trex_skull.obj");

        //__M = shell::load_obj("assets/kitty.obj");
        //__M = shell::load_obj("assets/elephant.obj");
    ...
    for (int i = 0; i < target.size() - 1; i++) {
      ...
      */
      ...
      std::vector<vec3> calc_rod_twist_force(real t, real cN, real cQ)
      {
        ...
        std::vector<vec3> Nr0 = _knotted_surface->get_rod_normals(R, M, cN * _eps);
        std::vector<vec3> Nr1 = _knotted_surface->get_rod_normals_from_surface(R, M, cN * _eps);
#if 1
        for (int i = 0; i < x_r.size(); i++)
        {
          gg::geometry_logger::line(x_r[i], x_r[i] + 1e-2 * Nr0[i], vec4(1.0, 0.0, 0.0, 1.0));
          gg::geometry_logger::line(x_r[i], x_r[i] + 1e-2 * Nr1[i], vec4(0.0, 1.0, 0.0, 1.0));
        }
#endif

        // use std::transform to map dot product over the two vectors
        std::vector<real> Ndiff(Nr0.size(), 0.0);
        for (int i = 0; i < x_r.size() - 1; i++)
        {

          vec3 x0 = x_r[i];
          vec3 x1 = x_r[i + 1];
          vec3 dx = (x1 - x0).normalized();
          vec3 N0 = Nr0[i];
          vec3 N1 = Nr1[i];

          // Calculate the signed angle between N0 and N1 with respect to the tangent dx
          vec3 crossProduct = N1.cross(N0);
          float dotProduct = N1.dot(N0);
          float angle = atan2(crossProduct.norm(), dotProduct);
          if (crossProduct.dot(dx) < 0)
          {
            angle = -angle;
          }
        ...
        std::vector<vec3> fv = calder::vortex_force(R, x_s, Ndiff, cQ * _eps, 6.0);
        // scale fv by scalar c
        std::transform(fv.begin(), fv.end(), fv.begin(), [t](const vec3 &a)
                       { return a * t; });
#if 0
        for (int i = 0; i < x_r.size() - 1; i++)
        {
          ...
          vec3 N = Nr1[i];
          vec3 x0 = x_r[i];
          vec3 T = (x_r[i + 1] - x_r[i]).normalized();
          vec3 B = T.cross(N).normalized();
          real r = 5e-2;
          std::array<vec3, 8> arc;
          for (int i = 0; i < 8; i++)
          {
            real ti = thet * (real(i) / 8.0);
            arc[i] = r * (cos(ti) * N + sin(ti) * B);
          }
          for (int j = 0; j < 7; j++)
          {
            gg::geometry_logger::line(x0 + arc[j], x0 + arc[j + 1], vec4(1.0, 0.0, 1.0, 1.0));
          }
        ...
        {
          // visualize tha arc subtended by the angle
          gg::geometry_logger::line(x_s[i], x_s[i] + 1e-2 * fv[i], vec4(0.0, 0.0, 1.0, 1.0));
        }
#endif
        return fv;
      }
                                                 ...
                                                 real t)
      {
        ...
        std::vector<vec3> Nr = _knotted_surface->get_rod_normals_from_surface(R, M, cN * _eps);
    ...
    for (vec3 &N : Nss) {
      ...
      std::vector<vec3> calc_cyclide_normal_flow(real cN, real cQ)
      {
        ...
        std::vector<real> Q =
            calder::darboux_cyclide_sdf(R, Nr, x_s_f, N_s_f, cQ * _eps, 6.0);

        // std::vector<real> Q =
        //     calder::darboux_cyclide_sdf_quadric_centers(R, Nr, x_s, N_s_v, cQ * _eps, 6.0);
      ...
      void morph_rod_step(real h, real t)
      {
        ...
        // Xr},
        //                                  1.0);
        hepworth::block::init_pinned(*__R, x_tt, constraints, x_r, 1e-2, {Xr});
      ...
      void step(int frame)
      {
        ...
        if (frame < 3000)
        {
          ...
          real t = calc_frame_t(frame, 0, 2400);
          real offset = 1.0;
          offset = min(1.0 + 20.0 * t, 20.0);
          std::cout << "offset: " << offset << std::endl;
          //_knotted_surface->set_helicity_constraint(true);
          //_knotted_surface->set_helicity_weight(2.0e-1);
          _knotted_surface->set_rod_offset(offset);
          _knotted_surface->set_willmore_weight(2.0e-1);
          _knotted_surface->set_area_weight(2.0e-1);
          _knotted_surface->set_shell_strain_weight(0.0e-2);
          _knotted_surface->set_shell_bending_weight(0.0e-2);
          ...
          */
          //_knotted_surface->add_shell_force(calc_rod_twist_force(-7e-2, 1.0, 4.0));
          _knotted_surface->add_shell_force(calc_cyclide_normal_flow(1.5, 1.5),
                                            0.5 / _h);
