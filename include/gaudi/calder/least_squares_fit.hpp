...
namespace gaudi
{
  ...
  namespace calder
  {
    ...
    using weight_func = real (*)(int, int, const std::vector<calder::datum::ptr> &, typename M_TYPE::Sum_Type::Node_Type, const vec3 &, const vec3 &, const vec3 &);
    ...
    weight_func<rod_bundle> rod_id_func = identity_weight<rod_bundle>;
    weight_func<shell_bundle> shell_id_func = identity_weight<shell_bundle>;
    ...
    weight_func<rod_bundle> rod_convex_weight = convex_weight<rod_bundle>;
    weight_func<shell_bundle> shell_convex_weight = convex_weight<shell_bundle>;
                                                               ...
                                                               weight_func = identity_weight<M_TYPE>)
    {
      ...
      std::vector<type> us = M_TYPE::integrate(
              ...
              const typename M_TYPE::Sum_Type::Tree &tree) -> type
          {
            ...
            vec3 dp = pj - pi;
            real wf = weight_func(i, j, data, node_type, dp, Ni, Nj);
            real dist = dp.norm();
            real kappa = computeK(dist, l0, p);
            real w_total = wj * kappa * wf;
                               ...
                               real k_r = 0.5)
    {
      ...
      return generic_fit<albers::quadric, rod_bundle>(
          R, Ns, p_pov, N_pov, l0, p, rod_id_func);
    }
                               ...
                               real p = 3.0)
    {
      ...
      return generic_fit<albers::quadric, shell_bundle>(
          M, Ns, p_pov, n_pov, l0, p, shell_id_func);
    }
    ...
    */
                             ...
                             real p = 3.0)
    {
      ...
      weight_func<shell_bundle> convex_weight =
          [](int i, int j, const std::vector<calder::datum::ptr> &data, shell_node_type node_type, const vec3 &dp, const vec3 &Ni, const vec3 &Nj)
      {
        return dp.dot(Nj) > 0 ? 1.0 : 0.0;
      };
