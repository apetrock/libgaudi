...
template <typename T>
Eigen::Matrix<T, 4, 1>
rotate_to_plane(Eigen::Matrix<T, 4, 1> normP, Eigen::Matrix<T, 4, 1> norm,
                Eigen::Matrix<T, 4, 1> cen, Eigen::Matrix<T, 4, 1> vec) {
  norm.normalize();
  normP.normalize();
  ...
  Eigen::Quaternion<T> dq(cos(o / 2.), sin(o / 2.) * xe[0], sin(o / 2.) * xe[1],
                          sin(o / 2.) * xe[2]);
...
template <typename T>
Eigen::Quaternion<T> naive_slerp(const Eigen::Quaternion<T> &input,
                                 const Eigen::Quaternion<T> &target, T amt) {
...
template <typename T>
Eigen::Matrix<T, 4, 1> blend(T t, Eigen::Matrix<T, 4, 1> p00,
                             Eigen::Matrix<T, 4, 1> pNorm,
  ...
  if (theta < 0.01) {
    ...
    T o = 0.10;
    Eigen::Quaternion<T> dq(cos(o / 2.), sin(o / 2.) * pNorm[0],
                            sin(o / 2.) * pNorm[1], sin(o / 2.) * pNorm[2]);
...
public:
  bool add(surf_ptr ob, long i, long j, T teni, T tenj) {
    ...
    Eigen::Quaternion<T> q1(ob->face(j)->normal());
    q1.normalize();

    vector<coordinate_type> f0 = coordinate_trace(fi);
    vector<coordinate_type> f1 = coordinate_trace(fj);
    ...
    for (long ii = 0; ii < Ni - 1; ++ii) {
      ...
      Eigen::Quaternion<T> dq(cos(o / 2.), sin(o / 2.) * xe[0],
                              sin(o / 2.) * xe[1], sin(o / 2.) * xe[2]);
      //				Eigen::Quaternion<T> dq = (qi*q0);
      ////dq[0]*=M_PI; dq.normalize();
      ...
      while (!at_head) {
        at_head = itb == ite;
        coordinate_type c = get_coordinate(itb);
        coordinate_type cp0 = dq.rotate(c - cen);

        set_coordinate(itb, cp0 + (mCurve.mKnots[ii + 1]));
        itb = itb->next();
      }
    ...
    graph_skinning<SPACE>
        gs; // TODO: stitch_faces needs to get moved to a surgery object
  ...
  bool slerp_face(T t, face_ptr fi, face_ptr fj) {
    ...
    while (!at_head) {
      ...
      coordinate_type ci = get_coordinate(itb) - ceni;
      coordinate_type cip = get_coordinate(itb->next()) - ceni;
      coordinate_type cj = rotate_to_plane(Ni, Nj, cenj, get_coordinate(jtb));
      // mOrient.push_back(ci + ceni);
      // mOrient.push_back(cj + ceni);
      coordinate_type cij = blend(t, ci, Ni, cj);
      set_coordinate(itb, cij + ceni);
