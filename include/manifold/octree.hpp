...
public:	
  ...
  bool operator()(const Octree<T,POINT_TYPE> &o, void *data){
    bool out = true;
    T r = o.radius();
    Eigen::Matrix<T,3,1> cen = o.center();
...
public:
			...
			const unsigned int currentDepth)
  {
    ...
    while (itb != ite) {
      ...
      const Eigen::Matrix<T,3,1> &c = icenter;
...
// the center of the child's node
	...
	Eigen::Matrix<T,3,1>	ncenter = icenter + offset;
  ...
  Eigen::Matrix<T,3,1>&	center()		{		return _center;	} 
  Eigen::Matrix<T,3,1>	center() const	{		return _center;	} 
