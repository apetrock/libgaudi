...
namespace m2 {
  ...
  template <class SPACE> //unfortunately I can't make this any more generic, but that's OK
  class merge_proc {
    M2_TYPEDEFS;
  public:
    typedef Octree<T,vertex_type> Vtree;

    void merge_vertices(control_ptr obj_in, vertex_ptr v1, vertex_ptr v2, 
      ...
      size_t sz = va2.size();
      fvl_iterator itb = va2.begin();  fvl_iterator ite = va2.end(); 
      for (itb; itb != ite; itb++) {
    ...
    bool operator()(const Vtree &o, control_ptr data){
      return this->merge(o,data);
    }

    bool merge(const Vtree &o, control_ptr data){
      control<SPACE>* obj = (control<space3>*)data;
  ...
  class obj_loader{
    ...
    obj_loader(){}
    ~obj_loader(){}
    m2::control<SPACE>& operator()(string s){
      return this->loadfile(s);
    }
    //1. load file into memory
    //2. instantiate manifold object
    //3. load up vertices in the object
    //4. load up faces, for every vertex in the face, make a face vertex and insert that into the appropriate vertex
    //5. for every vertex loop around all the face vertices and see if there are prev/next faces that share an adjacent vertex
    //	 if so, make an edge
    typedef Octree<T,vertex_type> Vtree;
    Vtree* build_octree(control_ptr in_){
    ...
    void merge_vertices(control_ptr in_, vector<list<face_vertex_ptr> > & corners){
      Vtree* oct = this->build_octree(in_);
      merge_proc<SPACE> merge;
      merge.corners = corners;
      traverse<Vtree, merge_proc<SPACE>, control_type>(*oct, merge, in_);
      //oct->traverse(merge,(void*) in_);
    }
		
    coordinate_type find_min(control<T>* in){
      in->pack();
      ...
      for(int i = 0; i < verts.size(); i++){
  	out[0] = verts[i]->coordinate()[0] < out[0] ? verts[i]->coordinate()[0] : out[0];
  	out[1] = verts[i]->coordinate()[1] < out[1] ? verts[i]->coordinate()[1] : out[1];
  	out[2] = verts[i]->coordinate()[2] < out[2] ? verts[i]->coordinate()[2] : out[2];
      }
      return out;
    }
        
    coordinate_type find_max(control<T>* in){
      in->pack();
      vector<vertex_ptr>& verts = in->get_vertices();
      coordinate_type out;
      coordinate_type avg;
  	...
  	if (c[0] > out[0]) out[0] = c[0];
  	if (c[1] > out[1]) out[1] = c[1];
  	if (c[2] > out[2]) out[2] = c[2];
      }
      return out;
    }

    void stitch_edges(control_ptr obj, vertex_ptr vert, list<face_vertex_ptr> & rFaceVertices){
      fvl_iterator itb = rFaceVertices.begin(); fvl_iterator ite = rFaceVertices.end();
      
      for(itb; itb != ite; itb++) {
	fvl_iterator jtb = rFaceVertices.begin(); fvl_iterator jte = rFaceVertices.end();
	face_vertex_ptr fvn = *itb;
	if(fvn->edge()) continue;
	for(jtb; jtb != jte; jtb++) {
	  face_vertex_ptr fvp = *jtb;
	  
	  if(fvp->prev()->edge()) continue;                 
    ...
    control_ref buildObj(vector<coordinate_type> & inputVerts,
			 vector<vector<int> > & inputFaces){

      control_ptr obj = new control_type();
	...
	//calcTriNormals();
      ...
      control_ref obj =  buildObj(inputVerts, inputFaces);
  ...
  template<typename SPACE>
  void writeOBJ(m2::control<SPACE>& obj, string filename)
  {
    M2_TYPEDEFS;
	...
	if(vertices[x]->position_in_set()==14637) vertices[x]->print();
	fout << "v " << vertex[0] << " " << vertex[1] << " " 
	     << vertex[2] << endl;
      }
    ...
    for (unsigned int x = 0; x < faces.size(); x++){
      ...
      fout << "f " 
	   << vert0->position_in_set()+1<< " "
	   << vert1->position_in_set()+1<< " "
	   << vert2->position_in_set()+1<< endl;
    }
  ...
  template<typename SPACE>
  void writePBRT(m2::control<SPACE>& obj, const char* filename)
  {
    M2_TYPEDEFS;
    ...
    for (unsigned int x = 0; x < faces.size(); x++){
      ...
      fprintf(file, "%i %i %i\n", 
	      ...
	      vert1->position_in_set(),
	      vert2->position_in_set());
    }
    fprintf(file, "]\n");
