...
public:
	...
	void firstImpulse(vector<coordinate_type> &vels, T eps)
	{
		...
		for (int i = 0; i < points.size(); i++)
		{
			...
			T d = this->minLength;
			getAllNearest<SPACE, triangle_type, coordinate_type>(ci, tree, tris, collectedTris, 2.0 * d);
	...
	vector<contact_manifold> getCollisions(const vector<coordinate_type> &vels)
	{
		...
		for (int i = 0; i < points.size(); i++)
		{
			...
			int indexMin = 0;
			coordinate_type cmin;
			getAllNearest<SPACE, swept_triangle_type, swept_point_type>(li, tree, tris, collectedTris, 0.025 * this->minLength);
...
#pragma omp parallel
					  ...
					  vertex_ptr v2)
	{
		...
		fv0->prev() = fv2;
		fv0->vertex() = v0;
		v0->add_face_vertex(fv0);
		fv1->vertex() = v1;
		v1->add_face_vertex(fv1);
		fv2->vertex() = v2;
		v2->add_face_vertex(fv2);
	...
	debug.DebugLines0.push_back(minEdgeSet[i+1]);
		...
		for (int i = 0; i < minEdgeSet.size(); i += 2)
		{
			...
			if (m2::intersectLineTest<SPACE, triangle_type>(ep0, ep1, exteriorTree, oldFaces))
				return;
			if (m2::intersectLineTest<SPACE, triangle_type>(ep0, ep1, interiorTree, newFaces))
				return;
		}
		face_vertex_ptr fv1b = f1->fbegin();
		face_vertex_ptr fv1e = f1->fend();
		...
		for (int i = 0; i < newTriangles.size(); i += 3)
		{
			...
			int ctype = 0;
			if (newTriangles[i + 0]->face() == newTriangles[i + 1]->face())
				ctype = 0;
			else
				ctype = 1;
			...
			int ptype = 0;
			if (newTriangles[ip + 0]->face() == newTriangles[ip + 1]->face())
				ptype = 0;
			else
				ptype = 1;
				  ...
				  vector<triangle_type> &corners)
	{
		...
		for (int i = 0; i < ppairs.size() / 2; i++)
		{
			...
			if (m2::intersectLineTest<SPACE, triangle_type>(ep0, ep1, faceTree, corners))
				hit = true;
			if (hit)
				return;
		}
...
public:
	...
	calc_sdf(control_ptr targetMesh)
	{
	...
	vector<pair<T, coordinate_type>> getSdf(vector<coordinate_type> &coordinates)
	{
		...
		{
			int cId =
				getNearest<SPACE, triangle_type>(coordinates[i], this->tree, this->tris);
			coordinate_type N;
			T d;
...
public:
	...
	void relax_mesh_no_collapse()
	{
		...
		vector<coordinate_type> endPoints;
...
public:
	...
	{
		vector<coordinate_type> u;
		u.resize(mMesh->get_vertices().size(), coordinate_type(0, 0, 0, 1.0));
		...
		while (stack.size() > 0)
		{
			...
			coordinate_type chargeNet(0, 0, 0, 0.0);
			T netChargeMag = 0;
			int N = pNode.size;
			T iN = 1.0 / (T)N;
			int beg = pNode.begin;
		...
		T thresh = 0.5;
		vector<vertex_ptr> &verts = mMesh->get_vertices();
		//TIMER traversal//TIMER("traversing tree");
		//#ifdef _OPENMP
		//#pragma omp parallel for
		//#endif
		for (int i = 0; i < verts.size(); i++)
	...
	vector<coordinate_type> integrateVelocity(T dt)
	{
		...
		for (int k = -off; k < off + 1; k++)
			for (int j = -off; j < off + 1; j++)
				...
				{
					coordinate_type ijk(i * dx, j * dx, k * dx, 0.0);
					T rad = ijk.norm();
		...
		for (int k = -off; k < off + 1; k++)
			for (int j = -off; j < off + 1; j++)
				for (int i = -off; i < off + 1; i++)
				{
					T rad = norm(coordinate_type(i * dx, j * dx, k * dx, 0.0));
					if (rad <= cutOff * regLength)
					{
		...
		vector<coordinate_type> u;
		u.resize(mMesh->get_vertices().size(), coordinate_type(0, 0, 0, 0));
		int blockAvg = 0;
		int blockCount = 0;
						...
						for (int k = 0; k < sz; k++)
						{
								...
								z + zo < zRes && z + zo > -1)
							{
								...
								for (int j = bStart1; j < bEnd1; j++)
								{
									...
									if (!compatible)
										continue;
									if (ei->v1()->face()->calc_area() < 0.1 * minLength * minLength)
										continue;
									if (ei->v2()->face()->calc_area() < 0.1 * minLength * minLength)
										continue;
									if (ej->v1()->face()->calc_area() < 0.1 * minLength * minLength)
										continue;
									if (ej->v2()->face()->calc_area() < 0.1 * minLength * minLength)
										continue;
