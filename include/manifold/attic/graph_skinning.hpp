...
public:
  ...
  surf_ptr build(m1::control<T> *graph_in) {
    ...
    faces_on_vertex.resize(
        graph_in->get_vertices()
            .size()); // some micromanagement here is necessary.
    ...
    for (int i = 0; i < N2; i++) {
      ...
      de = graph_in->E(i)->v2()->coordinate() -
           graph_in->E(i)->v1()->coordinate();
      ...
      */
      ...
      face_list &fov1 =
          faces_on_vertex[i1]; // list because we will be pushing back data
      face_list &fov2 =
    ...
    for (long i = 0; i < faces_on_vertex.size(); i++) {
      ...
      if (fov.size() > 1) {
        ...
        typename std::list<std::pair<face_ptr, face_ptr>>::iterator pb =
            jpairs.begin();
        typename std::list<std::pair<face_ptr, face_ptr>>::iterator pe =
            jpairs.end();
        int N3 = 0;
        while (pb != pe) {
  ...
  bool opposing_face_match(face_ptr f1, face_ptr f2, T tol) {
    ...
    bool norm_match = false, vert_match = false, size_match = false;
    T d = dist(n1, n2);
    if (d >= 2. - tol) {
      norm_match = true;
      if (f1->size() == f2->size()) {
