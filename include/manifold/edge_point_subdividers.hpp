...
public:
  void inset_vertex(surf_ptr in, vertex_ptr v) {
    ...
    while (iterating) {
      iterating = fvb != fve;
      coordinate_type de = get_coordinate(fvb->next()) - get_coordinate(v);
      // right now do 1/3 in next make it variable
      coordinate_type nvc = get_coordinate(v) + 0.45 * de;
      vertex_ptr nv = new vertex_type();
      set_coordinate(nv, nvc);
      face_vertex_ptr nfv = new face_vertex_type();
      va.push_back(nv);
      fva.push_back(fvb);  // I pick this up in an array because topological
                           // changes ruin connectivity
      nfva.push_back(nfv); // I pick this up in an array because topological
                           // changes ruin connectivity
      fvb = fvb->vnext();
    }
    ...
    for (int i = 0; i < N; i++) {
      ...
      oldFace1->next() = oldFace2;
      oldFace2->prev() = oldFace1;
      oldFace0->next() = oldFace1;
      oldFace1->prev() = oldFace0;
      newFace1->next() = newFace0;
      newFace0->prev() = newFace1;
  ...
  void inset_vertex_triangle_collapse(surf_ptr in, vertex_ptr v) {
    ...
    while (iterating) {
      ...
      if (fvb->coface()->size() > 3) {
        coordinate_type de = 0.5 * (get_coordinate(fvb->next()) +
                                    get_coordinate(fvb->vnext()->next())) -
                             get_coordinate(v);
        // right now do 1/3 in next make it variable
        coordinate_type nvc = get_coordinate(v) + 0.1 * de;
        vertex_ptr nv = new vertex_type();
        set_coordinate(nv, nvc);
        face_vertex_ptr nfv = new face_vertex_type();
        va.push_back(nv);
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (oldFace0->coface()->size() > 3) {
        ...
        newFacei0->next() = newFacei1;
        newFacei1->prev() = newFacei0;
        newFacei1->next() = newFacei2;
        newFacei2->prev() = newFacei1;
        newFacei2->next() = newFacei0;
        newFacei0->prev() = newFacei2;
  ...
  void inset_vertex_flag_collapse(surf_ptr in, vertex_ptr v,
                                  vector<coordinate_type> &lc, long lcb) {
    face_ptr nf = new face_type();
    face_vertex_ptr fvb = v->fbegin();
    face_vertex_ptr fve = v->fend();
    std::vector<vertex_ptr> va;
  ...
  bool subdivide_set(surf_ptr in, list<vertex_ptr> &lv) {
    ...
    while (vi != ve) {
      ...
      while (iterating2) {
        iterating2 = fvb != fve;
        if (fvb->coface()->flag != 3) {
          coordinate_type c1 = get_coordinate(fvb);
          coordinate_type c2 = get_coordinate(fvb->vnext()->next());
          coordinate_type de = 0.5 * (c1 + c2) - get_coordinate(v);
          // right now do 1/3 in next make it variable
          coordinate_type nvc = get_coordinate(v) + 0.15 * de;
