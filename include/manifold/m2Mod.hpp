...
template <typename T>
al::Vec<3, T> rotate_to_plane(al::Vec<3, T> normP, al::Vec<3, T> norm,
                              al::Vec<3, T> cen, al::Vec<3, T> vec) {
  norm.normalize();
  normP.normalize();
  ...
  al::Quat<T> dq(cos(o / 2.), sin(o / 2.) * xe[0], sin(o / 2.) * xe[1],
                 sin(o / 2.) * xe[2]);
...
template <typename T>
al::Vec<3, T> rotate_to_xy(al::Vec<3, T> norm, al::Vec<3, T> cen,
                           al::Vec<3, T> vec) {
  al::Vec<3, T> vz(0, 0, 1);
  norm.normalize();
...
template <typename T>
al::Vec<3, T> rotate_from_xy(al::Vec<3, T> norm, al::Vec<3, T> cen,
                             al::Vec<3, T> vec) {
  al::Vec<3, T> vz(0, 0, 1);
  norm.normalize();
  al::Vec<3, T> xe = -cross(norm, vz).normalize();
...
template <typename T>
al::Quat<T> naive_slerp(const al::Quat<T> &input, const al::Quat<T> &target,
                        T amt) {
  al::Quat<T> result;
...
al::Vec<3, T> blend(T t, al::Vec<3, T> p00, al::Vec<3, T> pNorm,
  ...
  if (theta < 0.01) {
    ...
    T o = 0.10;
    al::Quat<T> dq(cos(o / 2.), sin(o / 2.) * pNorm[0], sin(o / 2.) * pNorm[1],
                   sin(o / 2.) * pNorm[2]);
...
public:
  bool add(surf_ptr ob, long i, long j, T teni, T tenj) {
    ...
    for (long ii = 0; ii < Ni - 1; ++ii) {
      ...
      //					at_head = itb == ite;
      //					coordinate_type c =
      // itb->coordinate(); 					coordinate_type N
      // = fi->normal(); 					T o =
      // 0.5*t*M_PI; 					al::Quat<T> qb(cos(o*0.5), sin(o*0.5)*N[0],
      // sin(o*0.5)*N[1], sin(o*0.5)*N[2]);
      // qb.normalize(); 					al::Vec<3,T> cp1 = qb.rotate(c-cen);
      //					//
      // al::Vec<3,T> cp1 = cp0;
      //					//
      // itb->coordinate() = cp0 + (mCurve.mKnots[ii] + mCurve.mKnots[ii +
      // 1])*0.5; 					itb->coordinate() = cp1 +
      // (mCurve.mKnots[ii+1]); 					itb =
      // itb->next();
      //				}
  ...
  bool slerp_face(T t, face_ptr fi, face_ptr fj) {
    ...
    while (!at_head) {
      ...
      //				coordinate_type ci =
      // rotate_to_xy(Ni,ceni,itb->coordinate());
      ...
      //
      //				coordinate_type cij =
      // blend(t,ci,al::Vec<3,T>(0,0,1),cj);
