...
namespace m2 {
  ...
  struct face_bin{ 
	...
	if(f){
	  ...
	  for (int i = 0; i < bRes[0]; i++) {
		...
		int ix = mini[0] + i; int iy = mini[1] + j; int iz = mini[2] + k;
		if(ix < _xRes && ix > -1 &&
		   iy < _yRes && iy > -1 &&
		   iz < _zRes && iz > -1){
	...
	+ coordinate_type(x*_dx,y*_dx,z*_dx) + 0.5*coordinate_type(_dx,_dx,_dx);
      return cen;
    }
  ...
  struct edge_bin{ 
	...
	if(x > _xRes-1 || y > _yRes-1 || z > _zRes-1) continue;
	int index = x + y*_xRes + z*_xRes*_yRes;
	if(phase == 0){
	  _binCounter[index]++;
	}
  ...
  public:
    ...
    pole_node(const pole_node & rhs){
      ...
      level  = rhs.level;
      children = new int[8];
      for(int i = 0; i < 8; i++) children[i] = rhs.children[i];
    }
  ...
  public:
    ...
    void calcHalfCenter(coordinate_type & half, 
      ...
      for(int i = beg; i < beg + N; i++) com += points[permutation[i]];
      com /= (T)N;
      coordinate_type min = com;      
      coordinate_type max = com;
      for(int i = beg; i < beg + N; i++) {
	coordinate_type p = points[permutation[i]];
	for(int j = 0; j < 3; j++) min[j] = min[j] < p[j] ? min[j] : p[j];
	for(int j = 0; j < 3; j++) max[j] = max[j] > p[j] ? max[j] : p[j];
      }
      half = 0.5*(max - min);
      cen  = 0.5*(max + min);
    }
    ...
    void build(vector<coordinate_type> & points,
      ...
      while(stack.size() > 0){
	int pNodeId = stack.top(); stack.pop();
  ...
  public:
    ...
    void calcHalfCenter(coordinate_type & half, 
      ...
      for(int i = beg; i < beg + N; i++) com += points[permutation[i]].center();
      com /= (T)N;
      coordinate_type min = com;      
      coordinate_type max = com;
      for(int i = beg; i < beg + N; i++) {
  ...
  public:
	...
	nodes.push_back(root);
      ...
      while(stack.size() > 0){
	frame_type cFrame = stack.top(); stack.pop();
	...
	if(nodes[pNodeId].level == maxLevel)  continue;
	...
	if(nodes[pNodeId].data.size() > maxLeafSize){
	  ...
	  for(int i = 0; i < 8; i++){
	    ...
	    T yh = 0.5*half[1];
	    T zh = 0.5*half[2];
	    if(i == 0) offset = coordinate_type(-xh, -yh, -zh);
	    if(i == 1) offset = coordinate_type( xh, -yh, -zh);
	    if(i == 2) offset = coordinate_type(-xh,  yh, -zh);
	    if(i == 3) offset = coordinate_type( xh,  yh, -zh);
	    if(i == 4) offset = coordinate_type(-xh, -yh,  zh);
	    if(i == 5) offset = coordinate_type( xh, -yh,  zh);
	    if(i == 6) offset = coordinate_type(-xh,  yh,  zh);
	    if(i == 7) offset = coordinate_type( xh,  yh,  zh);
  ...
  inline bool intersectLineTest(typename SPACE::coordinate_type e0, typename SPACE::coordinate_type e1, 
    ...
    std::stack<int> cstack; cstack.push(0);
    bool hit = false;
  ...
  typename SPACE::double_type getNearestRange(typename SPACE::coordinate_type & p,
	...
	if      (cnode.children[0] == -1) cstack.push(cnode.children[1]);
	else if (cnode.children[1] == -1) cstack.push(cnode.children[0]);
	else{
	  int pDim = cnode.dim;
	  int cDim = (pDim + 1)%3;
  ...
  struct ccd{
    ...
    T edgeEdge(line_type e12, 
	       line_type e34){
      //Ec = b
      Eigen::MatrixXd E(2,2);  E = Eigen::MatrixXd::Zero(2,2);
