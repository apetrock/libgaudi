...
public:
  int planeBoxOverlap(CTYPE normal, CTYPE vert, CTYPE maxbox) {
    ...
    // std::cout << " min/max: " << vmin.transpose() << " " << vmax.transpose()
    // << std::endl; std::cout << " normal: " << normal.transpose() <<
    // std::endl;
...
#define AXISTEST_Z0(a, b, fa, fb)                                              \
  ...
  bool triBoxOverlap(const CTYPE boxcenter, const CTYPE boxhalfsize,
    ...
    //	normal = ( e0.y*e1.z - e0.z*e1.y, e0.z*e1.x - e0.x*e1.z, e0.x*e1.y -
    // e0.y*e1.x );
    normal = cross(e0, e1);
    d = dot(normal, v0); /* plane eq: normal.x+d=0 */
    // std::cout << " norm, v0: "  << normal.transpose() << " " <<
