...
public:
  ...
  edge_ptr connect_face(surf_ptr obj_in, face_vertex_ptr v1,
    ...
    edge_ptr e1 = new edge_type();
    size_t flag = 0;
    std::cout << " faces" << face1 << " " << face2 << std::endl;
    if (face1->size() == 1) {
      flag += 1;
    }
  ...
  bool delete_degenerates(surf_ptr obj, face_ptr f, real eps) {
    if (f->is_degenerate()) {
      if (f->get_front()->edge())
