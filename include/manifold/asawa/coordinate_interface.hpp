...
typename SPACE::real norm(typename surf<SPACE>::vertex_ptr v0,
  ...
  return va::norm(coordinate_type(c0 - c1));
}
...
typename SPACE::real geometric_mean_length(typename surf<SPACE>::surf_ptr s) {
  ...
  for (auto e : s->get_edges()) {
    if (!e)
      continue;
    sum += log(asawa::ci::length<SPACE>(e));
    N++;
  }
  sum /= real(N);
  return exp(sum);
...
typename SPACE::real mean_length(typename surf<SPACE>::surf_ptr s) {
  ...
  int N = 0;
  for (auto e : s->get_edges()) {
    real dist = asawa::ci::length<SPACE>(e);
    if (!e)
      continue;
    // std::cout << N << " " << dist << std::endl;
    if (dist == 0)
      std::cout << "zero" << std::endl;
    sum += asawa::ci::length<SPACE>(e);
    N++;
  }
...
std::vector<TYPE> get(asawa::surf<SPACE> *surf,
...
template <typename SPACE, typename TYPE>
void set(asawa::surf<SPACE> *surf, const std::vector<TYPE> &vals,
         typename SPACE::vertex_index id) {
  int i = 0;
  for (auto v : surf->get_vertices()) {
...
get_vertex_areas(typename surf<SPACE>::surf_ptr s) {

  std::vector<typename SPACE::real> areas;

  auto verts = s->get_vertices();
  for (auto v : s->get_vertices()) {
    areas.push_back(area<SPACE>(v));
  }
...
template <typename SPACE>
std::vector<typename SPACE::real>
get_face_areas(typename surf<SPACE>::surf_ptr s) {

  std::vector<typename SPACE::real> areas;

  auto faces = s->get_faces();
  for (auto f : faces) {
    areas.push_back(area<SPACE>(f));
  }
...
get_lines(std::vector<typename surf<SPACE>::edge_ptr> edges) {
  ...
  for (auto e : edges) {
    if (!e)
      continue;
    line_type line = asawa::ci::get_line<SPACE>(e);
...
std::vector<V> faces_to_verts(std::vector<V> face_vals,
  ...
  for (int i = 0; i < verts.size(); i++) {
    ...
    asawa::for_each_vertex<SPACE>(
        verts[i], [&r, &face_vals](face_vertex_ptr fv) {
