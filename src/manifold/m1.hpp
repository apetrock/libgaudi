...
namespace m1 {
  ...
  public:
    M1_TYPEDEFS
    control(){}
    control(m2::control<T>& in){
      in.pack();
      in.pack_vertices();
      vector<m2::vertex<T>*> V = in.get_vertices();
      vector<m2::edge<T>*> E = in.get_edges();			
      for (long i = 0; i < V.size(); i++) {
	m2::vertex<T>* vc = V[i];
	vector<m2::face_vertex<T>*> vfv = vc->get_face_vertices();
	long sz = vfv.size();
    ...
    virtual edge_ref  non_redundant_connect(long i1, long i2){
      ...
      for(int i = 0; i < ea1.size(); i++){
	if(ea1[i])
	  if(ea1[i]->other(v1) == v0) hasV0 = true;
      }
	...
	if(ea0[i])
	  if(ea0[i]->other(v0) == v1) hasV1 = true;
      }
	...
	if (mVertices[i] != NULL) {
	  ...
	  else glColor3f(v0->drawColor[0], v0->drawColor[1], v0->drawColor[2]);
	  glVertex3f(c1[0],c1[1],c1[2]);
	  glEnd();
...
#endif
#if 1
	  T denom = v0->mCovarianceVal[0] + v0->mCovarianceVal[1] + v0->mCovarianceVal[2];
 	  for(int g = 0; g < 3; g++){
	    coordinate_type dp0 = c1;
	    coordinate_type dp1 = c1 + 0.005*v0->mCovarianceVal[g]/denom*v0->mCovarianceVec[g];
  ...
  public:
    ...
    edge(vertex_ref o_vertex, vertex_ref d_vertex,
	 size_t o_sock, size_t d_sock){
    ...
    ~edge(){
      cout << "destroying the edge: " << this << endl;
    }
    ...
    size_t stack_place() const{
      return placement_in_stack;
    }
  ...
  class vertex_iterator {
    M1_TYPEDEFS
    public:
    vertex_iterator(vertex_ptr in){cur_vertex = in; vert_index = 0;}
    vertex_ptr& operator*()       { return cur_vertex->get_edge(vert_index)->other(cur_vertex);}		
    vertex_ptr  operator*() const { return cur_vertex->get_edge(vert_index)->other(cur_vertex);}		
    vertex_iterator& operator++()   {vert_index++; return *this;}
    vertex_iterator  operator++(int){vert_index++; return *this;}		
    vertex_iterator& operator--()   {vert_index--; return *this;}
    vertex_iterator  operator--(int){vert_index--; return *this;}		
    bool operator==(vertex_iterator& other){ bool out = this->vert_index == other.vert_index; return out;}
    bool operator!=(vertex_iterator& other){ bool out = this->vert_index != other.vert_index; return out;}
    bool operator<=(vertex_iterator& other){ bool out = this->vert_index <= other.vert_index; return out;}
    bool operator< (vertex_iterator& other){ bool out = this->vert_index <  other.vert_index; return out;}
    bool operator>=(vertex_iterator& other){ bool out = this->vert_index >= other.vert_index; return out;}
  ...
  public:
	...
	for (long i = 0; i < mEdges.size(); i++) {
	  if(mEdges[i]){ 
	    ...
	    tEdges.push_back(ei);
	    if(ei->v0() == this){
	      tEdges.back()->v0_socket() = j; j++;
	    } else {
	      tEdges.back()->v1_socket() = j; j++;
	    }
	  }
	}
	swap(mEdges,tEdges);              
    ...
    vertex_iterator<T>& end()  { vertex_iterator<T>* out = new vertex_iterator<T>(this); out->set_end()  ; return *out;}
    vertex_iterator<T>& begin(){ vertex_iterator<T>* out = new vertex_iterator<T>(this); out->set_begin(); return *out;}		
    ...
    virtual void set_current_socket(size_t sock_in){
      cur_socket = sock_in;
    }
    ...
    edge_ptr get_edge(int ei){
      if(mEdges[ei]) return mEdges[ei];
      else
	{
	  int eCount = 0; int retrieve = 0;
	  for(int i = 0; i < mEdges.size(); i++){
	    if(mEdges[i]) eCount++;
	    if(eCount == ei) retrieve = i;
	  }
    ...
    virtual void push_singular_edge(edge_ptr new_edge){
      ...
      for (long i = 0; i < mEdges.size(); i++) {
	if (new_edge->v0() == mEdges[i]->v0() && new_edge->v1() == mEdges[i]->v1()) {
	  is_singular = false;
	}
	if (new_edge->v1() == mEdges[i]->v0() && new_edge->v0() == mEdges[i]->v1()) {
	  is_singular = false;
	}
      }
      if (is_singular) {
	...
	std::cout << " deallocating: " << mEdges[this_sock]->position_in_set() << std::endl;
    ...
    virtual bool  is_connected(size_t socket) const { 
      size_t out  =  true;
      if(mEdges.size() < socket) out = false;
      return out;
    }
    ...
    virtual vertex_ref get_connected(size_t sock_num){
      ...
      if (cur_edge->incoming(this)) {adj_vertex = cur_edge->v0();}
      else adj_vertex = cur_edge->v1(); 
