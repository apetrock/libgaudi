...
namespace m2 {	
  ...
  public:
    ...
    inline coordinate_type periodicTangent(vector<coordinate_type> & path, int i){
      ...
      int N  = path.size();
      int im2 = i-2; im2 = im2 < 0 ? im2 + N : im2;
      int im1 = i-1; im1 = im1 < 0 ? im1 + N : im1;
      int ip1 = i+1; ip1 = ip1 > N-1 ? ip1 - N : ip1;
      int ip2 = i+2; ip2 = ip2 > N-1 ? ip2 - N : ip2;

      coordinate_type	
	t = -path[ip2] + 8.0*path[ip1] - 8.0*path[im1] + path[im2];
      return t;
    }
    ...
    inline quat parallelTransport(vector<coordinate_type> & path, 
					quat & q0, int i){
    ...
    inline quat rotorFromFrame(const coordinate_type & t,
  ...
  public:
    ...
    edge_ptr insert_edge(control_ptr	obj_in,
			 vertex_ptr	v1,
			 vertex_ptr	v2)
    {
    ...
    edge_ptr split_face(control_ptr    obj_in,
			face_vertex_ptr	v1,
			face_vertex_ptr v2){
      ...
      v1->prev() = v2c; v2c->next() = v1;			
      v2->prev() = v1c; v1c->next() = v2;
    ...
    edge_ptr  connect_face(control_ptr      obj_in,
			   face_vertex_ptr  v1,
			   face_vertex_ptr  v2){
	...
	//v1 & v2 are both face vertices
	v1->next() = v2; v2->prev() = v1;
	v1->prev() = v2; v2->next() = v1;
	e1->set(v1,v2);
	v1->face() = face1;
	v2->face() = face1;	
      }
    ...
    face_ptr  delete_edge(control_ptr	obj_in,
			   edge_ptr 	e)
    {
    ...
    face_ptr  join_face(control_ptr obj_in, edge_ptr e){
	 ...
	 //	 std::cout << e->position_in_set() << " " << v1->position_in_set() << " " << v2->position_in_set() << " "
	 //	 << v1a << " " << v2a << " " << " " << v1p << " " << v2n << std::endl;
	 v1p->next() = v2n; v2n->prev() = v1p;
	 v2p->next() = v1n; v1n->prev() = v2p;
	 f1->fbegin() = v1p;
    ...
    list<edge_ptr> stitch_unlike_faces(control_ptr obj_in, face_ptr f1, face_ptr f2){
      list<edge_ptr> new_edges;
      face_vertex_ptr 
	fv1  = f1->fbegin(),
	fv1e = f1->fend(),
	...
	fvb = v->fbegin();
	fve = v->fend();
	//iterate and grab edge pointers flagging them so as to not delete them twice
	...
	fve = v->fend();
      ...
      face_vertex_ptr this_wing, inco_edge; //we'll need this after connecting everything up
      //1. we need to loop around and gather the current vertex's next and the next vertex's previous;
      face_ptr nf = new face_type(); int j = 0;
      iterating = true;
	...
	this_wing->face() = nf;
	inco_edge->face() = nf;
	inco_edge->position_in_face() = j; j++;
	this_wing->position_in_face() = j; j++;
...
bool delete_degenerates(control_ptr obj, vertex_ptr v){
      ...
      int dSize = 0;
      auto gatherDegenVerts = 
	[&vertDegenerates](face_vertex_ptr fv)->void{
	if(fv->next()->vertex()->calc_size() < 3) {
	  vertDegenerates.push_back(fv->next()->vertex());
	}
      };

      auto gatherDegenEdges = 
	[&edgeDegenerates](face_vertex_ptr fv)->void{
	if(fv->face()->size() < 3) {
      ...
      std::for_each(edgeDegenerates.begin(), edgeDegenerates.end(), 
		    [this, &obj](edge_ptr e) mutable {
		      this->delete_edge(obj, e);
		    });
      std::cout << edgeDegenerates.size() << " " 
		<< vertDegenerates.size() << std::endl;
      return (edgeDegenerates.size() > 0 || vertDegenerates.size() > 0);
    };
    
    vertex_ptr collapse_edge_triangle(control_ptr obj, edge_ptr e){
      if(e->v1()->vertex() == e->v2()->vertex()) return e->v1()->vertex();
	...
	std::cout << va0 << " " << va1 << " " << va2 << " "
		  << vb0 << " " << vb1 << " " << vb2 << std::endl;
	...
	fvb = fvb->vnext();
      ...
      v1->remove_face_vertex(fv1); delete fv1;
      v2->remove_face_vertex(fv2); delete fv2;
    ...
    void flip_edge(control_ptr & obj, edge_ptr & e){
      ...
      if(e1p->v1() == fv1p) e1p->v1() = fv2;
      else e1p->v2() = fv2;

      if(e2p->v1() == fv2p) e2p->v1() = fv1;
      else e2p->v2() = fv1;
    ...
    bool stitch_faces_diff_size(control_ptr obj_in, face_ptr f1, face_ptr f2, T tol){
    ...
    bool bevel_face(control_ptr  obj_in, face_ptr  f1, 
                    const T & offset, 
                    const T & inset){
      f1->update_normal();
	...
	coordinate_type c = fvb->coordinate();
        ...
        coordinate_type C = c-cen; C.normalize();
	...
	fvb->vertex() = nv;
	//and add this fv to the vertex, and in turn add the new vertex to the set
	...
	*/
	...
	if (ed->v1() == fvb) {ed->v1() = fv3;}
	else                 {ed->v2() = fv3;}
                
	fv3->edge() = ed;
                
	//now we add these new face vertices to their respective vertices.
	fv0->vertex() = nv0; nv0->add_face_vertex(fv0); fv0->face() = nf;
	fv1->vertex() = nv1; nv1->add_face_vertex(fv1); fv1->face() = nf;
	fv2->vertex() = nv2; nv2->add_face_vertex(fv2); fv2->face() = nf;
	fv3->vertex() = nv3; nv3->add_face_vertex(fv3); fv3->face() = nf;
	nf->size() = 4;
	//in order to connect up the sides, we'll have to loop around the top and
	//bottom vertices
	topfv.push_back(fv2); topfv.push_back(fv1);
	botfv.push_back(fv3); botfv.push_back(fv0);
	nf->update_normal();
	...
	ne->v1() = fvtp; fvtp->edge() = ne;
	ne->v2() = fvbm; fvbm->edge() = ne;
	obj_in->push_edge(ne); 
      }
	...
	edge_ptr ne = new edge_type();                
	ne->v2() = fvb; fvb->edge() = ne; 
	ne->v1() = fvo; fvo->edge() = ne;
	fvb = fvb->next(); i+=2;
	obj_in->push_edge(ne);
      }
      return true;
    ...
    bool incremental_patch_hole(face_vertex_ptr & fv,
				control_ptr obj){
	...
	return 0;
      ...
      face_vertex_ptr fv1;
      face_vertex_ptr fv2;
      if(!fv0->next()->edge()) fv1 = fv0->next();
      else fv1 = fv0->next()->vnext();
      if(!fv1->next()->edge()) fv2 = fv1->next();
      else fv2 = fv1->next()->vnext();
      ...
      nfv0->next() = nfv1;  nfv1->prev() = nfv0;
      nfv1->next() = nfv2;  nfv2->prev() = nfv1;
      nfv2->next() = nfv0;  nfv0->prev() = nfv2;
    ...
    bool patch_hole(face_vertex_ptr fv,
		    control_ptr obj){
      face_vertex_ptr fvb = fv;
      face_vertex_ptr fve = fv->vprev();
      if(!fve) 
	...
	return 0;
      ...
      while(!atHead && iterations < maxIterations)
      	{
	  if(!fvb->edge()){
	    //std::cout << fvb << " " << fvb->next()->vnext() << " " << fvb->vertex() <<" " << fve  << std::endl;
	    fvb->face()->color.r = 0;
	    fvb->face()->color.g = 0.5;
	    fvb->face()->color.b = 0.5;
	    if(!fvb->next()->edge()) fvb = fvb->next();
	    else fvb = fvb->next()->vnext(); // i wish I could draw pictures of all of this.
	    loopSize++;
	  }
	  else fvb = fvb->vnext();
	  atHead = fvb == fve;
	...
	while(!atHead)
	  {
	    ...
	    std::cout << " fvb "<< fvb << " has edge: " << fvb->edge() << std::endl;
	...
	fvl_iterator jtb = rFaceVertices.begin(); fvl_iterator jte = rFaceVertices.end();
	for (jtb; jtb != jte; jtb++) {
