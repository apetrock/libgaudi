...
namespace m2 {
  ...
  class convex_hull  {
    ...
    /*TODO: doesn't work perfectly yet, there is still a bug somewhere, it might be a precision thing */
    M2_TYPEDEFS
    public:
    ...
    control_ptr tetrahedron(coordinate_type cxu,
			    coordinate_type cxl,
			    coordinate_type cyu,
			    coordinate_type cyl,
			    coordinate_type czu,
      ...
      max_pts.push_back(cxu);			max_pts.push_back(cxl);
      max_pts.push_back(cyu);			max_pts.push_back(cyl);
      max_pts.push_back(czu);			max_pts.push_back(czl);			
    ...
    void collect_points(face_ptr f,
			coordinate_list& clist,
    ...
    void concave_flip(control_ptr obj){
	...
	if(det < 0){
	  ...
	  edge_ptr e = edges[i];
	  m2::modify<SPACE> mod; e = mod.flip_edge(e);
	}
      }				
    }		
    ...
    void find_horizon(face_ptr&			face_in,
		      coordinate_type	cen,
	...
	face_ptr cface = f_stack.back(); f_stack.pop_back();
    ...
    control_ptr quick_hull(std::vector<coordinate_type> clist){
	...
	avg+=clist[i];
      ...
      cxu = avg; cxl = avg; cyu = avg; cyl = avg; czu = avg; czl = avg;
      for (long i = 0; i < clist.size(); i++) {
	if (clist[i][0]>cxu[0]) {
	  cxu = clist[i];
	}
	...
	if (clist[i][2]<czl[2]) {
      ...
      control_ptr out = tetrahedron(cxu, cxl, cyu, cyl, czu, czl);
      manage<SPACE> man;
	...
	if (faces_in_horizon.size() > 0) {
	  ...
	  //remove the old old faces from the set and merge their points with the current face's
	  std::cout << "removing faces and merging points size: " << faces_in_horizon.size() << std::endl;
	  ...
	  while (iterating1) { //we need to capture all the faces we just stellated
