...
template <typename SPACE, typename BLOCK_T>
  struct LeafBlock{ 
    M2_TYPEDEFS;
  ...
  template <typename SPACE, int N, typename BLOCK_T>
  struct Block{ 
    M2_TYPEDEFS;
    ...
    void setCenter(coordinate_type cen){
      mcenter = cen;
    }
	...
	+ coordinate_type(x*mdx,y*mdx,z*mdx,0) + 0.5*coordinate_type(mdx,mdx,mdx,0);
      return cen;
    }
    ...
    virtual BLOCK_T * allocate(int x, int y, int z){
      ...
      if(!mchildMask[index]){
        ...
        //         << " - " << x << " " << y << " " << z <<  " " <<  nblock << std::endl; 
	
        mblocks[index] = nblock;
        return nblock;
      }
      else return mblocks[index];
    }
    ...
    virtual bool get(int x, int y, int z, 
		     BLOCK_T * block, 
		     std::function<bool(coordinate_type, T, void*)> cb, 
		     void * data ){
      int index = x + y*mxRes + z*mxRes*myRes;	
      //std::cout << "  get: " << x << " " << y << " " << z << std::endl;
      //std::cout << "  res: " << mxRes << " " << myRes << " " << mzRes << std::endl;
	...
	pos - (mcenter - 0.5*mlengths + 0.5*coordinate_type(mdx,mdx,mdx, 0.0));
      bini[0] = binf[0]/mdx + 0.5;
      bini[1] = binf[1]/mdx + 0.5;
      bini[2] = binf[2]/mdx + 0.5;
    }

    vector<int> intersectingBins(bounding_box<SPACE> & bbox){
    }
  ...
  void testBins(){    
    ...
    std::cout << (i2 + (i1 << logNx2) + (i0 << logNx1 << logNx2)) << " " << std::endl;
  ...
  class BlockTree : public Block<SPACE, L0, Block<SPACE, L1, Block<SPACE,L2, LeafBlock<SPACE, BLOCK_T> > > >{

    M2_TYPEDEFS;
  public: 
    
    typedef Block<SPACE, L0, Block<SPACE, L1, Block<SPACE,L2, LeafBlock<SPACE,BLOCK_T> > > >  Root;
    ...
    virtual bool topGet(int x, int y, int z, Leaf *& leaf, 
      ...
      //int nTotal = 
      if(x < 0       || y < 0       || z < 0 ||
         x >= msSize || y >= msSize || z >= msSize) return false;
      Vec3i xi = getLevels(x);
      Vec3i yi = getLevels(y);
      Vec3i zi = getLevels(z);
      ...
      Root & root = *this;
      if(!cb(Root::center(xi[0],yi[0],zi[0]), Root::dx(), data)) return false;
      //std::cout << " b1 " << std::endl;
      Block1 * b1 = Root::get(xi[0], yi[0], zi[0]);
      b1->setCenter(Root::center(xi[0], yi[0], zi[0]));
      b1->setLength(Root::dx());

      if(!cb(b1->center(xi[1],yi[1],zi[1]), b1->dx(), data)) return false;      
      //std::cout << " b2 " << std::endl;
      Block2 *    b2 = b1->get(xi[1], yi[1], zi[1]);
      b2->setCenter(b1->center(xi[1], yi[1], zi[1]));
      b2->setLength(b1->dx());
      
      if(!cb(b2->center(xi[2],yi[2],zi[2]), b2->dx(), data)) return false;      
      leaf = b2->get(xi[2], yi[2], zi[2]);
      //std::cout << "dx: " << b2->dx() << std::endl;
      //std::cout << " leaf: "  << xi[2] << " " <<  yi[2] << " " <<  zi[2] << " " << leaf << std::endl;
      return true;
    }

    coordinate_type decFloor(coordinate_type c, T dx ){
      //std::cout << " decFllor"<< c[0] << " " << dx << " "<< c[0]*dx <<  " " << floor(c[0]/dx) << " " << floor(c[0]/dx)*dx << std::endl;
      return coordinate_type(floor(c[0]/dx)*dx,
			     floor(c[1]/dx)*dx,
			     floor(c[2]/dx)*dx,1);
    }

    coordinate_type decCeil(coordinate_type c, T dx ){
      //std::cout << " decCeil"<< c[0] << " " << dx << " " << ceil(c[0]/dx) << " " << ceil(c[0]/dx)*dx << std::endl;
      return coordinate_type(ceil(c[0]/dx)*dx,
			     ceil(c[1]/dx)*dx,
			     ceil(c[2]/dx)*dx,1);
    }
  ...
  BlockTree<SPACE, L0, L1, L2,std::vector<m2::vertex<SPACE>*> >* 
    makeTree(m2::control<SPACE> *  mesh){
    M2_TYPEDEFS;
    typedef BlockTree<SPACE, L0, L1, L2,std::vector<m2::vertex<SPACE>*> >  vertex_tree;
    ...
    //makeTriTree(m2::control<SPACE> *  mesh, ImmediateLines * debug, ImmediateLines * debug0){
    makeTriTree(m2::control<SPACE> *  mesh){
    
      M2_TYPEDEFS;
    typedef BlockTree<SPACE, L0, L1, L2,std::vector<m2::face<SPACE>*> >  vertex_tree;
    typedef typename vertex_tree::Root   Root;
    typedef typename vertex_tree::Block1 Block1;
    typedef typename vertex_tree::Block2 Block2;
    ...
    auto tribox = [&](coordinate_type cen, T half, void * data){
      ...
      //debug->pushBox(xyz(cen), xyz(halfc));
#if 0
	...
	for(int y = binb[1]; y < bine[1]; ++y)
	  for(int x = binb[0]; x < bine[0]; ++x){
	    Leaf * leaf = NULL;
	    int b[3] = {x,y,z};
            if(block->topGet(b[0], b[1], b[2], leaf, tribox, static_cast<void*>(tri))){
