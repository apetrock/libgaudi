...
interface PathTestModule extends WasmModule, LoggerWasmModule {
  PathTest: new () => PathTestInstance;
}

interface PathTestInstance extends WasmLoggerAPI {
  // Animation control
  step(): void;
  reset(): void;
...
function SimulationController({ 
  ...
  wasmInstance: PathTestInstance | null;
  isPlaying: boolean;
...
export function PathTest() {
  const instanceFactory = useCallback((module: PathTestModule) => {
    return new module.PathTest();
  }, []);

  const { loading, error, module, instance } = useWasmModule<PathTestModule, PathTestInstance>(
    'path_test',
    instanceFactory
  );
  ...
  const handleDownloadRecording = () => {
    if (recordingBlob) {
      ...
      a.href = url;
      a.download = `path-constraint-test-${Date.now()}.webm`;
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Build the module with: npm run wasm:build
          </p>
        </div>
      </div>
    );
    ...
    <div className="h-full flex flex-col">
      ...
      <Card className="mb-4">
        <CardHeader>
          ...
          <CardDescription>
            Test path constraints with logger visualization and video recording
          </CardDescription>
        </CardHeader>
        <CardContent>
