 ...
 */
export function Foo_demo() {
  const [module, setModule] = useState<Foo_demoModule | null>(null);
  ...
  useEffect(() => {
    const loadWasm = async () => {
      try {
        setLoading(true);
        const instance = await loadWasmModule('foo_demo') as Foo_demoModule;
        setModule(instance);
        setLoading(false);
      } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p>Loading foo_demo WASM module...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Foo_demo</h1>
        <p className="text-gray-600">A simple foo_demo WASM demo</p>
      </div>
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-lg">
        ...
        <ul className="text-sm space-y-1">
          <li>• <code>wasm/foo_demo/src/foo_demo.cpp</code> - C++ source</li>
          <li>• <code>wasm/foo_demo/foo_demo_endpoints.ts</code> - TypeScript interface</li>
          <li>• <code>src/components/Foo_demo.tsx</code> - React component</li>
        </ul>
      </div>
    </div>
  );
