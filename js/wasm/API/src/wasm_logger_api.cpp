...
void wasm_logger_point(float x, float y, float z, float r, float g, float b, float a) {
  wasm_logger::point(vec3(x, y, z), vec4(r, g, b, a));
}

void wasm_logger_line(float x0, float y0, float z0, float x1, float y1, float z1, 
                      float r, float g, float b, float a) {
  wasm_logger::line(vec3(x0, y0, z0), vec3(x1, y1, z1), vec4(r, g, b, a));
}

void wasm_logger_box(float cx, float cy, float cz, float hx, float hy, float hz,
                     float r, float g, float b, float a) {
  wasm_logger::box(vec3(cx, cy, cz), vec3(hx, hy, hz), vec4(r, g, b, a));
}

void wasm_logger_ext(float min_x, float min_y, float min_z, 
                     float max_x, float max_y, float max_z,
                     float r, float g, float b, float a) {
  wasm_logger::ext(vec3(min_x, min_y, min_z), vec3(max_x, max_y, max_z), vec4(r, g, b, a));
}

void wasm_logger_frame_wrapper(const emscripten::val& M, float cx, float cy, float cz, float C) {
  ...
  wasm_logger::frame(matrix, vec3(cx, cy, cz), C);
}

void wasm_logger_clear() {
  wasm_logger::clear();
}

void wasm_logger_render() {
  wasm_logger::render();
}
...
// Emscripten bindings
EMSCRIPTEN_BINDINGS(wasm_logger_module) {
  ...
  // Bind the wasm_logger data access functions
  ...
  // Bind the wasm_logger drawing functions
  function("wasm_logger_point", &gaudi::wasm_logger_point);
  function("wasm_logger_line", &gaudi::wasm_logger_line);
  function("wasm_logger_box", &gaudi::wasm_logger_box);
  function("wasm_logger_ext", &gaudi::wasm_logger_ext);
  function("wasm_logger_frame", &gaudi::wasm_logger_frame_wrapper);
  function("wasm_logger_clear", &gaudi::wasm_logger_clear);
  function("wasm_logger_render", &gaudi::wasm_logger_render);
}